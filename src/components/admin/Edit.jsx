...
const Edit = ({ product, editItemModal, setEditItemModal,updateItem }) => {
    ...
    // when product prop changes (user clicked Edit), update local state
    useEffect(() => {
        if (product) {
            setItem(product);
        }
    ...
    function handleSubmit(e) {
        ...
        if (item.rating === "" || item.rating === null) {
            ...
            valid = false;
        } else if (Number(item.rating) < 1 || Number(item.rating) > 5) {
        ...
        if (valid) {
            updateItem(item); // update in AdminHome
            setEditItemModal(false); // close modal
    ...
    return (
        <>
            ...
            <div className={`h-screen   w-full md:w-1/2 bg-white fixed top-0 right-0 z-50 transform transition-transform duration-500 
                    ${editItemModal ? "translate-x-0" : "translate-x-full"} overflow-y-auto `}>
                ...
                <div className='w-[100%] h-[100%] flex justify-center'>
                    <form action="" className='w-[80%] p-2 flex flex-col items-start gap-4 mt-10' onSubmit={handleSubmit}>
                        ...
                        <div className='w-[100%] grid grid-cols-2 justify-items-center mt-5 '>
                            ...
                            <button type="submit" className='w-[75%] p-[5px] py-2 border-[1px] shadow-md rounded-lg text-white font-bold text-[1.2rem] bg-green-500 hover:bg-green-700 hover:text-white'>Edit Item</button>
                        </div>
